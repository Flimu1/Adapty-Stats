...
### 3.3 Что нужно выписать из документации / Postman

Заполните и сохраните (для реализации):

| Пункт | Где смотреть | Ваше значение / заметка |
|-------|----------------|---------------------------|
| Базовый URL API (например `https://api.adapty.io`) | Документация / Postman | |
| Путь к эндпоинту "Retrieve analytics data" (например `/v1/analytics/export`) | API Reference / Postman | |
| Тело запроса: как передать приложение (app_id / application_id / и т.д.) | Описание операции | |
| Тело запроса: как задать период (даты от/до, или «последние 24ч») | Описание операции | |
| Формат ответа: JSON или CSV? Структура (поля MRR, revenue, installs и т.д.) | Пример ответа в доке / Postman | |
| Откуда брать **Installs** (тот же эндпоинт или другой, например cohort/conversion)? | Документация | |
