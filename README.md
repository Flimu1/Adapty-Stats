...
быть конечным и неотрицательным (дельты могут быть отрицательными), counts —
целыми; дневные Revenue и Installs не могут превышать MTD; conversion должен
...
- `⚠️ Проверка данных: обнаружено N проблем…` — недостоверные поля уже заменены
  на `N/A`, а безопасные детали отправлены администратору.
